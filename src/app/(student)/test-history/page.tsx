...
async function loadDrillCompletedFromDB(userId: string): Promise<DrillCompletedMap> {
  const { data } = await supabase.from('drill_records').select('answer_id, drill_mode, completed_at').eq('user_id', userId)
  const map: DrillCompletedMap = {}
  for (const r of (data || [])) {
    if (!r.answer_id) continue
    if (!map[r.answer_id]) map[r.answer_id] = {}
    map[r.answer_id][r.drill_mode] = { completed: true, lastAt: new Date(r.completed_at).getTime() }
  }
  return map
}

function openDrillInNewTab(answer: Answer, mode: string, sessionScore?: number, sessionId?: string) {
  const drillData = {
    answerId: answer.id,
    initialMode: mode,
    sessionId: sessionId || null,
    testScore: sessionScore || null,
    wrongAnswers: [{
...
export default function TestHistoryPage() {
  ...
  // Supabaseからドリル記録を読み込み
  const loadDrillStatus = async (uid: string) => {
    const map = await loadDrillCompletedFromDB(uid)
    setDrillCompleted(map)
  }
  ...
  useEffect(() => {
    async function load() {
      ...
      await loadDrillStatus(user.id)
      setLoading(false)
    }
    load()
  }, [router])

  // フォーカス時にドリル記録を再読み込み（別タブでドリル完了時）
  useEffect(() => {
    const onFocus = async () => {
      const { data: { user } } = await supabase.auth.getUser()
      if (user) await loadDrillStatus(user.id)
    }
  ...
  useEffect(() => {
    ...
    try {
      channel = new BroadcastChannel('drill-completion')
                                        ...
                                        <div className="mt-2 pt-2 border-t border-red-100">
                                          ...
                                          <div className="flex flex-col gap-1.5">
                                            {drillRecs.map(dr => {
                                              ...
                                              return (
                                                <button key={dr.key} onClick={() => openDrillInNewTab(a, dr.key, s.total_score, s.id)}
