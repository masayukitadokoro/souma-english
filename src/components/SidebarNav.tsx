...
import { ClipboardCheck, BarChart3, BookOpen, LogOut } from 'lucide-react'
          ...
          {NAV_ITEMS.map(({ label, icon: Icon, href }) => {
            ...
            return (
              <button
                key={href}
                onClick={() => router.push(href)}
                className={`flex flex-col items-center gap-0.5 px-3 py-1 ${active ? 'text-indigo-600' : 'text-gray-400'}`}
              >
                <Icon size={20} />
